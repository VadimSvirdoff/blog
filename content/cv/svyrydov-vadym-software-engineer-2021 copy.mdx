---
title: "Svyrydov Vadym Software Engineer"
publishedDate: "November 11, 2021"
slug: "svyrydov-vadym-software-engineer-2021"
description: "cv Svyrydov Vadym Software Engineer"
titleType: "link"
name: "Vadym Svyrydov"
---


# Software Engineer

## Summary

4 years of experience Frontend development with React, CMS and design systems

## Relevant experience

- healthcare organization (Sitecore CMS)
- collaboration service (Design systems)
- telecommunication service (Adobe experience manager CMS)
- legal service (Adobe experience manager CMS)

## Skills

### Hard skills

- Familiarity with CMS platforms such as Sitecore and Adobe experience manager
- Proficient in ReactJS, Redux, Next.js and Typescript
- Experience in testing applications using Jest and Cypress.
- Experience using Git and GitHub for version control and collaborative development
- Knowledge of release process, including continuous integration and deployment with tools like Azure, Gitlab, Github, Vercel
- Familiar with Cloud concepts AWS

### Soft skills

- English B2
- Familiarity with Agile methodologies and project management tools like JIRA

## Experience

### Softserve, August 2022 - now

### Project: Healthcare organization

#### Project description

Non-profit organization dedicated to improving patient outcomes through research, development, education, 

and clinical exellence in the field of musculoskeletal disorders.

Project included 10 members.

Our goal was migration on headless and redesign on web components.

#### Responsibilities

- Web components development;
- Migration from SXA to JSS;
- Demo's new features;
- New design integration;
- Unit tests development;					
- Estimation, prioritization, and distribution of high-quality solutions;			
- POC's implementation;

#### Tools & Technologies

React, Sitecore, Docker, Azure, Javascript, Typescript, CSS, HTML, SASS, Jira

### Softserve, December 2021 - August 2022

### Project: Collaboration service

#### Project description

Collaboration service is a digital workspace for visual collaboration.

It helps everyone on the team imagine together to unlock new ideas, solve hard problems, and innovate faster.

Design system project included about 15 members.

Our goal was accessible core application components development for Design System framework.

#### Responsibilities

- Development for Design System framework;
- Demo's new features;
- New design integration;
- Cypress tests development;					
- Estimation, prioritization, and distribution of high-quality solutions;			
- POC's implementation
- Accessibility development

#### Tools & Technologies

React, Stenciljs, Storybook, Javascript, Typescript, Cypress, Github, CSS, HTML, SASS, Jira

### Epam Systems, October 2020 - December 2021

### Project: Telecommunication service

#### Project description

One of the most prominent telecommunication services in the world.

This application allows users to buy tv, mobile, and internet packages.

The project included more than 100 people in different domains.

Daily site visits are more than 100,000 people. 

My team was involved in order tracker development.

This application was used for order flow tracking and included such fields as appointment date, order status, payment method, address, bill type, and frequency.

#### Responsibilities

- Key developer role;
- Product discovery;
- Demo's new features;
- New design integration;
- Lean tests development;
- Estimation, prioritization, and distribution of high-quality solutions;
- POC's implementation;
- Documentation maintenance;
- Legacy system migration to AWS;
- Headless React integration;
- Integrated new release branch strategy;

#### Tools & Technologies

React, Redux, Saga, Javascript, Typescript, Jest, Enzyme, GitLab, CSS, HTML, SASS, Jira

### Project: Legal service

#### Project description

Legal research platform for companies and government.

Customer is one of the oldest news agencies in the world.

Our team has worked with product billing flow.

This application allows users to buy legal solutions.

#### Responsibilities


- New design integration;
- Extended architecture for better code-splitting;
- Increase code coverage;
- Estimation, prioritization, and distribution of high-quality solutions;
- Mentoring;
- Pre-checks, and onboardings;
- Proposed lean test development strategy;

#### Tools & Technologies

React, Context, Fetch, Javascript, Typescript, Jest, Enzyme, GitHub, CSS, HTML, Jira

### Zazmic, May - August 2020

#### Project: Document flow SRM system

#### Project description

 SRM system for creating document flow which can be assigned to employee.

 After the assignment responsible person must follow steps like attach the required document, add personal information, and add expiration status.

 Finally, assignment owner receives all data from the document flow.

#### Responsibilities

- Migrating React context to Redux architecture
- Increase current architecture scalability
- Clean code refactoring
- New design integration;
- Application state normalization
- Mentoring

#### Tools & Technologies

React, Redux, Thunk, Javascript, Jest, Enzyme, GitHub, CSS, HTML, Trello

#### Project: Real Estate Marketplace

#### Project description

 Real Estate marketplace application for Miami.

 This application allows buying the property without commission.

#### Responsibilities

- Developing high-quality architecture from scratch
- Lottie animation integration
- Google Maps API integration
- Lazy loading and filters implementation
- Onboarding newcomers
- Collaboration with FE developer

#### Tools & Technologies

React, Redux, Thunk, Javascript, GitHub, SCSS, HTML, Trello

### Sigma Software, April 2019 - May 2020

#### Project: Tours marketplace

#### Project description

Tours marketplace for the USA.

This marketplace allows users to create/book a tour.

#### Responsibilities

- Development using geolocation API
- Huge module decomposition and refactoring
- Fixing performance issues and memory lick
- New design implementation
- Tours list implementation

#### Tools & Technologies

React, React Native, Redux, Saga, Javascript, GitHub, SCSS, HTML, Jira

#### Project: Artworks Marketplace

#### Project description

This service allows users to buy/sell their artworks. 

#### Responsibilities

- Layout development and bug fixes
- Unit tests development
- Participation in planning activities
- Collaboration with mentor

#### Tools & Technologies

React, Redux, Thunk, Typescript, Javascript, Jest, GitHub, SCSS, HTML

## Education

### Kharkiv National University of Radioelectronics
Master's degree, Radioelectronic Systems, 2017-2019

## Achivments

### Winner of Tezos Foundation Hackathon 2021
